...
const PricingCard: React.FC = () => {
  ...
  return (
    <div className="bg-white p-6 border border-gray-200 rounded-xl shadow-2xl sticky top-20 animated-card" style={{ animationDelay: '1.5s' }}>
      <h2 className="text-xl font-bold text-gray-800 mb-6">Tarifni Tanlang</h2>
      ...
      <div id="pricing-options" className="space-y-3 mb-6">
        {Object.keys(prices).map((key) => {
          ...
          return (
            ...
            >
              <div>
                <p className="font-semibold text-gray-800">{option} OYLIK</p>
      ...
      <div className="mb-6 p-3 border border-gray-300 rounded-xl flex justify-between items-center bg-white">
        <Input type="text" placeholder="Promokodni kiriting" className="w-full outline-none text-sm bg-transparent border-none focus-visible:ring-0" />
        <Button variant="ghost" className="text-lime-500 font-semibold text-sm hover:text-lime-600 p-0 h-auto">Promokod</Button>
      </div>

      <div className="flex justify-between items-center border-t pt-4">
        <p className="text-lg font-bold text-gray-800">Jami</p>
