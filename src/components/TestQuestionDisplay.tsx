...
import ListeningTestDisplay from "./ListeningTestDisplay"; // Yangi import
import ReadingTestDisplay from "./ReadingTestDisplay";   // Yangi import
import WritingTestDisplay from "./WritingTestDisplay";   // Yangi import

interface TestQuestionDisplayProps {
  currentSection: FetchedCEFRSection | undefined; // Yangi prop
  currentQ: FetchedCEFRQuestion | undefined; // Yangi tip
...
const TestQuestionDisplay: React.FC<TestQuestionDisplayProps> = ({
  ...
  if (currentPhase === "section_announcement" || currentPhase === "part_finished_announcement") {
    return (
      <div className="space-y-4 flex flex-col items-center justify-center min-h-[250px]">
        ...
        <h3 className="text-2xl font-bold text-primary dark:text-primary-foreground mt-4">
          {currentPhase === 'part_finished_announcement' ? t("add_question_page.part_finished") : t("cefr_test_page.prepare_for_section", { section: currentSection?.type })}
        </h3>
        <CountdownBar countdown={countdown} initialCountdown={initialCountdown} />
      </div>
    );
  ...
  switch (currentSection.type) {
    ...
    case "Speaking":
      // Speaking section logic (adapted from previous implementation)
      switch (currentQ.question_type) {
          ...
          return (
            <div className="space-y-4">
              <h3 className="text-xl font-semibold text-muted-foreground">
                {currentSection.type} - {t("add_question_page.question")} {currentQuestionIndex + 1}
              </h3>
              <CountdownBar label={currentPhase === "speaking_reading_question" ? t("add_question_page.reading") : t("add_question_page.answer")} countdown={countdown} initialCountdown={initialCountdown} />
        ...
        case "system_generated_part1_2":
          ...
          return (
            <div className="space-y-4">
              <h3 className="text-xl font-semibold text-muted-foreground">
                {currentSection.type} - {t("add_question_page.image")} {currentQuestionIndex + 1}
              </h3>
        ...
        case "system_generated_part2":
          ...
          return (
            <div className="space-y-4">
              ...
              <CountdownBar label={currentPhase === "speaking_preparation" ? t("add_question_page.preparation") : t("add_question_page.answer")} countdown={countdown} initialCountdown={initialCountdown} />
    ...
    default:
      return <p className="text-muted-foreground">{t("add_question_page.unknown_section_type")}</p>;
  }
};
