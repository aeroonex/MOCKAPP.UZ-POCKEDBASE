...
export default function Home() {
  ...
  const items = [
    {
      ...
      title: "Questions",
      subtitle: "Barcha savollarni ko'rish",
      path: "/questions",
    },
    {
      icon: <PlusCircle className="w-10 h-10" />,
      title: "Add Question",
      subtitle: "Yangi savol qo'shish",
      path: "/add-question",
    },
    {
      icon: <ListChecks className="w-10 h-10" />,
      title: "Mock Test",
      subtitle: "Testni boshlash",
      path: "/mock-test",
    },
    {
      icon: <Video className="w-10 h-10" />,
      title: "Records",
      subtitle: "Yozilgan videolarni ko'rish",
      path: "/records",
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-500 via-sky-500 to-slate-900 text-white p-6">
        ...
        <nav className="flex gap-6 text-lg">
          ...
          {(session || isGuestMode) && (
            <Link to="/login" onClick={handleLogout} className="hover:text-red-300 flex items-center gap-1">
              <LogOut className="h-4 w-4" /> {isGuestMode && !session ? "Guest Mode'dan chiqish" : "Logout"}
            </Link>
          )}
        </nav>
      </header>
      ...
      >
        <h2 className="text-5xl font-bold text-center mb-3 drop-shadow-md">Speaking Platform</h2>
        <p className="text-center text-slate-200 mb-16">Boshqaruv paneliga xush kelibsiz. Kerakli bo'limni tanlang.</p>
      </motion.div>
            ...
            <Link to={item.path}>
              <Card className="group bg-gradient-to-br from-white/20 to-white/5 backdrop-blur-2xl border border-white/30 rounded-2xl shadow-xl hover:scale-105 hover:border-indigo-400/50 hover:shadow-indigo-500/40 hover:shadow-2xl transition-all duration-500 cursor-pointer">
                <CardContent className="flex flex-col items-center text-center p-8">
                  ...
                  <Button className="mt-2 bg-indigo-500 hover:bg-indigo-600 text-white px-6 py-2 rounded-xl shadow-lg shadow-indigo-500/30 transition-all duration-300 hover:scale-105">Open</Button>
                </CardContent>
              </Card>
            </Link>
          </motion.div>
      ...
      <footer className="mt-20 text-center text-slate-200 opacity-90">
        ...
        <p className="text-lg mt-4">Bog'lanish uchun: <span className="font-bold text-white">+998 77 207 7117</span></p>
        <p className="text-sm mt-1 text-slate-300">24/7 qo'llab-quvvatlash xizmati</p>
      </footer>
    </div>
  );
}